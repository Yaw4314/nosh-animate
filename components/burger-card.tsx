  ...
  return (
    <div className="group relative h-full">
      <div className="bg-zinc-900 border border-white/5 rounded-[2.5rem] overflow-hidden h-full flex flex-col transition-all duration-500 hover:border-primary/30">
        {burger.popular && (
          <div className="absolute top-6 left-6 z-10 bg-primary text-black text-[10px] font-black px-4 py-1.5 rounded-full uppercase italic tracking-tighter shadow-xl">Best Seller</div>
        )}
        ...
        <div className="flex-1 p-8 flex flex-col bg-zinc-900/80 backdrop-blur-sm">
          ...
          <div className="flex items-end justify-between mt-auto">
            <div className="flex flex-col">
              {burger.originalPrice && <span className="text-zinc-600 text-xs font-black line-through italic mb-1">{formatPrice(burger.originalPrice)}</span>}
              <span className="text-3xl font-black text-primary italic tracking-tighter leading-none">{formatPrice(burger.price)}</span>
            </div>

            <Button
              onClick={handleAddToCart}
              className={`h-14 w-14 rounded-full p-0 transition-all duration-500 ${
                isAdded ? 'bg-green-500 text-white scale-110' : 'bg-primary text-black hover:bg-white hover:scale-110'
              }`}
            >
