...
export function WhatsAppCart({ isOpen, onClose }) {
  const { items, removeFromCart, updateQuantity, getTotalPrice } = useCart();
  const [orderType, setOrderType] = useState('delivery');
  const subtotal = getTotalPrice();
  const deliveryFee = orderType === 'delivery' ? 150 : 0;
  const total = subtotal + deliveryFee;
  const formatPKR = (v) => new Intl.NumberFormat('en-PK', { style: 'currency', currency: 'PKR', minimumFractionDigits: 0 }).format(v).replace('PKR', 'Rs.');

  const handleCheckout = () => {
    if (items.length === 0) return;
    let text = `*🍔 NOSH (Burgers & More) - NEW ORDER*%0A%0A`;
    items.forEach(i => { text += `*${i.name.toUpperCase()}*%0A  Qty: ${i.quantity} × ${formatPKR(i.price)}%0A  Subtotal: ${formatPKR(i.price * i.quantity)}%0A%0A`; });
